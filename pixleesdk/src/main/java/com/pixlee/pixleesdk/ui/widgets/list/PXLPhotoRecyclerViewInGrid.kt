...
open class PXLPhotoRecyclerViewInGrid : BaseRecyclerView {
    ...
    var listHeaderText: Spannable? = null

    fun initiate(gridSpan: Int = 2,
                 lineSpace: Space = Space(),
                 listHeaderText: Spannable? = null,
        ...
        addItemDecoration(GridSpacingItemDecoration(gridSpan, lineSpace.lineWidthInPixel, lineSpace.includingEdge, listHeaderText != null))
    ...
    internal override fun setList(type: ListAddType, list: List<PXLPhoto>, imageScaleType: PXLPhotoView.ImageScaleType, heightInPixel: Int) {
        clearOldList(type)
        if (pxlPhotoAdapter.list.isEmpty() && listHeaderText != null) {
            pxlPhotoAdapter.list.add(PXLPhotoAdapter.Item.Header(text = listHeaderText!!))
        }
        if (list.isNotEmpty()) {
            list.forEach {
    ...
    internal override fun setList(type: ListAddType, list: List<PhotoWithImageScaleType>) {
        ...
        if (pxlPhotoAdapter.list.isEmpty() && listHeaderText != null) {
            ...
            pxlPhotoAdapter.list.add(PXLPhotoAdapter.Item.Header(text = listHeaderText!!, padding = TextPadding(padding, padding, padding, padding)))
        }
        if (list.isNotEmpty()) {
            list.forEach {
