    ...
    fun bind(product: PXLProduct, isBookmarked: Boolean?, configuration: Configuration) {
        ...
        (product.price ?: 0.toBigDecimal()).let { price ->
            ...
            val salesPrice: Price? = product.salesPrice?.let { salesPrice ->
                if (product.hasAvailableSalesPrice()) {
            ...
            val total = if (noSalesPrice) {
                defaultPrinceString
            } else {
                "$salesIntegerPrice$salesDecimalPrice $defaultPrinceString"
            }
...
fun SpannableString.applyTextStyle(index: Int, textLength: Int, targetTextStyle: TextStyle, metrics: DisplayMetrics) {
    targetTextStyle.also {
        val fontSize = TypedValue.applyDimension(it.sizeUnit, it.size, metrics).toInt()
        setSpan(AbsoluteSizeSpan(fontSize), index, textLength, 0); // font size
        setSpan(ForegroundColorSpan(it.color), index, textLength, 0);// font color
    }
}

fun SpannableString.applyDisabledTextStyle(index: Int, textLength: Int, targetTextStyle: TextStyle, metrics: DisplayMetrics) {
    targetTextStyle.also {
        val fontSize = (TypedValue.applyDimension(it.sizeUnit, it.size, metrics) * 0.75).toInt()
        setSpan(AbsoluteSizeSpan(fontSize), index, textLength, 0) // font size
        setSpan(ForegroundColorSpan(lighten(it.color, .25)), index, textLength, 0) // font color
        setSpan(StrikethroughSpan(), index, textLength, 0) // font size
...
fun lighten(@ColorInt color: Int, fraction: Double): Int {
    ...
    Log.e("lighten", "=====================")
    Log.e("lighten", "lighten.red: $red")
    Log.e("lighten", "lighten.green: $green")
    Log.e("lighten", "lighten.blue: $blue")
