...
class PXLPhotoViewInRecyclerViewFragment : BaseFragment() {
    ...
    fun startList(cellSize:Int) {
        ...
        pxlPhoto?.also {
            ...
            for (i in 1..5) {
                list.add(PhotoWithImageScaleType(it, ImageScaleType.CENTER_CROP, cellSize))
            }
            ...
            for (i in 6..10) {
                //list.add("$i")
                list.add(PhotoWithImageScaleType(it, ImageScaleType.FIT_CENTER, cellSize))
            }

            list.add(PhotoWithImageScaleType(it, ImageScaleType.FIT_CENTER, 600.px.toInt()))
