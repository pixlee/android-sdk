 ...
 */
class KtxGalleryViewModel(val ktxBasicDataSource: KtxBasicDataSource, val ktxAnalyticsDataSource: KtxAnalyticsDataSource) : ViewModel() {
    ...
    val resultEvent: LiveData<Event<Command>>
        get() = _resultEvent

    sealed class Command {
        class Data(val isFistPage: Boolean, val list: List<PhotoWithImageScaleType>) : Command()
    ...
    fun getNextNext() {
        launchVMScope({
            searchSetting?.let {
                ...
                when (it) {
                    ...
                    if (it.photos.isNotEmpty()) {
                        ...
                        list.addAll(newList)
                        _resultEvent.value = Event(Command.Data(isFirstPage, newList))
                    }
            ...
            _resultEvent.value = Event(Command.Error)
            canLoadMore = true
            _loading.value = false
        })
    }
