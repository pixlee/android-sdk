...
public class SingleListViewItemActiveCalculator extends BaseItemsVisibilityCalculator {
    ...
    public SingleListViewItemActiveCalculator(Callback<ListItem> callback, List<? extends ListItem> listItems, boolean infiniteScroll) {
        mCallback = callback;
        mListItems = listItems;
        this.infiniteScroll = infiniteScroll;
    }

    int getRealListViewSize(int count) {
        if (mListItems == null && mListItems.size() == 0)
            return 0;

        if (infiniteScroll) {
            Log.e(TAG, "getRealListViewSize: " + mListItems.size());
    ...
    int getRealPosition(int scrollPosition) {
        if (mListItems == null && mListItems.size() == 0) {
            return 0;
        }

        if (infiniteScroll) {
            return scrollPosition % mListItems.size();
        } else {
    ...
    private void calculateMostVisibleItem(ItemsPositionGetter itemsPositionGetter, int firstVisiblePosition, int lastVisiblePosition) {
        ...
        if (SHOW_LOGS) Logger.v(TAG, "playcheck, item changed: " +mostVisibleItem.isMostVisibleItemChanged());
    ...
    private void topToBottomMostVisibleItem(ItemsPositionGetter itemsPositionGetter, int maxVisibilityPercents, ListItemData outMostVisibleItem) {
        ...
        for (int indexOfCurrentItem = itemsPositionGetter.getFirstVisiblePosition(), indexOfCurrentView = itemsPositionGetter.indexOfChild(outMostVisibleItem.getView())
            ...
            if (SHOW_LOGS) {
                Logger.v(TAG, "mListItems.size: " + mListItems.size()+ ", childSize:" + itemsPositionGetter.getChildCount());
                Logger.v(TAG, "topToBottomMostVisibleItem, indexOfCurrentItem " + indexOfCurrentItem + ", from: " + getRealPosition(itemsPositionGetter.getFirstVisiblePosition()));
                Logger.v(TAG, "topToBottomMostVisibleItem, indexOfCurrentView " + indexOfCurrentView + ", size:" + getRealListViewSize(itemsPositionGetter.getChildCount()));
            }
            ListItem listItem = mListItems.get(getRealPosition(indexOfCurrentItem));
            ...
            if (currentItemVisibilityPercents > mostVisibleItemVisibilityPercents) {
                ...
                outMostVisibleItem.fillWithData(getRealPosition(indexOfCurrentItem), currentView);
    ...
    private ListItemData getMockCurrentItem(ItemsPositionGetter itemsPositionGetter, int firstVisiblePosition, int lastVisiblePosition) {
        ...
        switch (mScrollDirection) {
            case UP:
                ...
                mockCurrentItemData = new ListItemData().fillWithData(getRealPosition(lastVisibleItemIndex), itemsPositionGetter.getChildAt(itemsPositionGetter.getChildCount() - 1));
                if (SHOW_LOGS) Logger.v(TAG, "getMockCurrentItem, scroll up " + lastVisibleItemIndex);
                break;
            case DOWN:
                mockCurrentItemData = new ListItemData().fillWithData(getRealPosition(firstVisiblePosition), itemsPositionGetter.getChildAt(0/*first visible*/));
                if (SHOW_LOGS) Logger.v(TAG, "getMockCurrentItem, scroll down " + firstVisiblePosition);
                break;
            default:
                throw new RuntimeException("not handled mScrollDirection " + mScrollDirection);
        }
    ...
    private void setCurrentItem(ListItemData newCurrentItem) {
        if (SHOW_LOGS) Logger.v(TAG, "playcheck setCurrentItem, newCurrentItem " + newCurrentItem);
